##############################################################################################
File Patch Code REDUX v0.95 [Sammi Husky feat. Grok]
##############################################################################################
...
string    "/ComboMode+/rsbe/"                   @ $80406920 # Sets path used for SD lookups / reads
string    "pf"                                  @ $80507b70
...
# Now includes path length debugging via OSReport.              #
#################################################################
HOOK @ $8001BF38
{
_start:
...
_main:
    ...
    mr      r28, r29                    # | copy original path pointer to r28
...
_skipAdjust:
    # Check if path matches "/menu/strap/StrapEn.pac"
    %lwi    (r3, 0x80507c40)            # | load target string address
    mr      r4, r28                     # | current path string
    %call   (strcmp)                    # | compare strings
    cmpwi   r3, 0                       # | check if strings match (strcmp returns 0 for match)
    bne     _continue                   # | skip if no match
    
    # Print match confirmation
    %lwi    (r3, 0x80507c20)            # | load match string address
    %call   (OSReport)                  # | call OSReport
    
    # Replace path with StrapEn2.pac
    %lwi    (r3, 0x80507c60)            # | load replacement string address ("/menu/strap/StrapEn2.pac")
    mr      r28, r3                     # | update r28 to point to replacement path
    
_continue:
    
    # Continue with original SD card check logic
    %FPCPath(0x08, r28)
    stw     r3, 0(r30)                  # \
